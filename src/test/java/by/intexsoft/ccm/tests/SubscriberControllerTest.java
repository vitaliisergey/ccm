...
public class SubscriberControllerTest {
    ...
    public void getAllPacksOfSubscriberWithParamsTest() throws Exception {
        ...
        mockMvc.perform(get("/subscriber/{id}/packs", 1)
                .param("limit", "5")
                .param("offset", "0"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
                .andExpect(jsonPath("$", hasSize(2)))
                .andExpect(jsonPath("$[0].packId", is(1)))
                .andExpect(jsonPath("$[0].name", is("Test")))
                .andExpect(jsonPath("$[1].packId", is(1)))
    ...
    public void connectPackToSubscriberTest() throws Exception {
        ...
        mockMvc.perform(post("/subscriber/{id}/packs/activate", "1").contentType(MediaType.APPLICATION_JSON_UTF8).content(
                jsonContent
        ))
                .andExpect(status().isOk())
    ...
    public void disconnectPackFromSubscriberTest() throws Exception {
        ...
        mockMvc.perform(post("/subscriber/{id}/packs/deactivate", "1").contentType(MediaType.APPLICATION_JSON_UTF8).content(
                jsonContent
        ))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
